...
export function StoresPage() {
  ...
  return (
    <div className="flex flex-col gap-6">
      {/* Hero */}
      <section className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-brand-deep via-[#0a4a10] to-brand-primary px-5 py-6 text-white shadow-lg sm:px-7 sm:py-8">
      ...
      <section className="overflow-hidden rounded-2xl border border-border/80 bg-card shadow-sm">
        <div className="flex flex-col gap-4 border-b border-border/70 bg-gradient-to-r from-brand-tint/40 via-background to-background px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-0.5 text-xs text-muted-foreground">
                Open a card to edit identity, media, and storefront layout
              </p>
            </div>
          </div>
          {isSuper ? (
            ...
            <ul className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
              {stores.map((store) => (
                <li key={store.id}>
                  <Link
                    to={`/stores/${store.id}`}
                    className="group flex h-full flex-col overflow-hidden rounded-2xl border border-border/80 bg-gradient-to-br from-background to-muted/20 transition-all hover:border-brand-primary/30 hover:shadow-md"
                  >
                    <div className="relative aspect-[16/9] bg-gradient-to-br from-brand-deep/90 via-[#0a4a10] to-brand-primary">
                      {store.banner?.path ? (
                        <img
                    ...
                    <div className="flex flex-1 flex-col gap-3 p-4">
                      ...
                      {isSuper && store.user ? (
                        <p className="truncate text-[11px] text-muted-foreground">
                          Owner · {store.user.first_name} {store.user.last_name}
                        </p>
                      ) : null}
